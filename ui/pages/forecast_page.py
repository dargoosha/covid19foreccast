...
from ui.components.charts import render_new_forecast_charts_tabs
from ui.components.analytics import render_analytics_block
...
"""
    ...
    if st.session_state["draft_file_name"] != current_name or st.session_state["draft_df"] is None:
        ...
        # запрет выбора первых 30 дней + учёт окна
        min_start = w + 30
        max_start = max(min_start + 1, n - 31)
    ...
    if df is None or len(df) == 0:
        st.error("Порожній датасет.")
        return

    st.success("Датасет завантажено. Налаштуйте параметри.")
    ...
    valid_dates = df[CFG.date_col].iloc[min_start:max_start]
    if len(valid_dates) == 0:
        st.error("Замало даних: потрібні щонайменше 30 днів історії перед стартом + вікно.")
        return
    ...
    render_analytics_block(results)
    ...
    df_full = df[[CFG.date_col, CFG.target_col]].rename(columns={CFG.target_col: "new_cases", CFG.date_col: "date"})
    render_new_forecast_charts_tabs(
        df_full=df_full,
        results=results,
        chosen_date_iso=chosen_date_iso,
        horizon=horizon_int,
...
"""
    ...
    st.subheader("Аналітика")
    render_analytics_block(by_type)
    ...
    # графік з історії залишаємо як є
    rows = []
    base_dates = None
